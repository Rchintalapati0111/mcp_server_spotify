...
async def demo_music_search():
    ...
    try:
        # Search for tracks
        result = await make_spotify_request("search", params={
            "q": "Taylor Swift", 
            "type": "track", 
            "limit": 5, 
            "market": MARKET
        ...
        for track in tracks:
            ...
            print(f"      Album: {album_name} | Duration: {duration} | Popularity: {track['popularity']}/100")
...
async def demo_artist_explorer():
    ...
    try:
        # Search for artist
        search_result = await make_spotify_request("search", params={
            "q": "The Beatles", 
            "type": "artist", 
            "limit": 1
        })
        ...
        if artists:
            ...
            # Get top tracks
            top_tracks = await make_spotify_request(f"artists/{artist_id}/top-tracks", 
                                                  params={"market": MARKET})
            print("   🎵 Top Tracks:")
            for i, track in enumerate(top_tracks.get("tracks", [])[:5], 1):
                duration = f"{track['duration_ms'] // 60000}:{(track['duration_ms'] // 1000) % 60:02d}"
                print(f"     #{i}. {track['name']} (Popularity: {track['popularity']}/100, Duration: {duration})")
                print(f"         Album: {track['album']['name']}")
            print()
            
            # Get recent albums
            albums = await make_spotify_request(f"artists/{artist_id}/albums", 
                                              params={"limit": 4, "market": MARKET})
            print("   💿 Recent Albums:")
            for album in albums.get("items", []):
                print(f"     • {album['name']} ({album['release_date'][:4]}) - {album['total_tracks']} tracks")
        else:
...
async def demo_album_discovery():
    ...
    try:
        # Search for albums
        albums_result = await make_spotify_request("search", params={
            "q": "Abbey Road", 
            "type": "album", 
            "limit": 4,
            "market": MARKET
        ...
        if albums:
            ...
            for album in albums:
                ...
                print(f"      Released: {album['release_date']} | Tracks: {album['total_tracks']}")
...
async def demo_browse_categories():
    ...
    try:
        result = await make_spotify_request("browse/categories", params={
            "country": MARKET, 
            "limit": 10
        })
        ...
        try:
            ...
            for item in valid_items:
                if search_type == "track":
                    ...
                    print(f"         💿 {item['name']} by {item['artists'][0]['name']} ({item['total_tracks']} tracks)")
                elif search_type == "playlist":
                    # Extra safety checks for playlist items
                    track_count = item.get('tracks', {}).get('total', 0) if item.get('tracks') else 0
                    owner_info = item.get('owner', {})
                    owner = owner_info.get('display_name', 'Unknown') if owner_info else 'Unknown'
                    playlist_name = item.get('name', 'Unknown Playlist')
                    print(f"         🎵 {playlist_name} by {owner} ({track_count} tracks)")
            print()
